...
# Every endpoint except /health requires the bearer token the gateway writes on
# first start. CLERQ_GATEWAY_TOKEN overrides the file, matching the gateway.
TOKEN="${CLERQ_GATEWAY_TOKEN:-}"
if [ -z "$TOKEN" ] && [ -f "$HOME/.clerq/gateway-token" ]; then
  TOKEN="$(tr -d '[:space:]' < "$HOME/.clerq/gateway-token")"
fi
if [ -z "$TOKEN" ]; then
  echo "No gateway token found. Start the gateway once, or set CLERQ_GATEWAY_TOKEN." >&2
  exit 1
fi
...
# Skills
if ! curl -sf "${AUTH[@]}" "$BASE/skills" > /dev/null; then
...
if curl -sf "${AUTH[@]}" -X POST "$BASE/calculate/eval" -H "Content-Type: application/json" -d '{"expression":"10 + 25","inputs":{}}' > /dev/null 2>/dev/null; then
...
# Filing prep (generic)
if curl -sf "${AUTH[@]}" -X POST "$BASE/filing/prep" -H "Content-Type: application/json" -d '{"form_type":"generic","data":{}}' > /dev/null 2>/dev/null; then
...
TASK_RESP=$(curl -sf "${AUTH[@]}" -X POST "$BASE/task" -H "Content-Type: application/json" -d '{"message":"Calculate 25% on 100 units"}' 2>/dev/null) || true
