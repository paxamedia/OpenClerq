...
the roadmap adds shell execution, git push and server installs, it becomes **a program that
executes model-generated instructions with local operating system authority**, and is
...
2. **Prompt injection via repo content** — a README, issue body, changelog or dependency description crafted to redirect the agent. Repo content is untrusted input, and the agent reads it by design.
3. **Prompt injection via pasted content** — the same threat through the chat console. Pasted text is untrusted, and the console will be able to draft scheduled jobs, so a model-drafted automation always saves disabled and pushes manually on its first run.
...
- A local attacker who already has the user's OS account. Anyone who can read `~/.clerq` as that user can read what the gateway can.
...
- **The kill switch stops everything in flight.** `POST /kill` refuses pending approvals, cancels running runs mid-call, and pauses triggers until `POST /resume`. `POST /runs/:id/cancel` stops a single run.
...
- **The vault master key lives in the OS keychain** — macOS Keychain, libsecret on Linux, DPAPI on Windows — generated on first use. `CLERQ_VAULT_KEY` still works, with a warning; a host with no keychain uses `~/.clerq/vault.key` (mode 0600).
- **Sandboxed processes have no network by default,** and an egress allowlist is enforced at the process boundary rather than in the agent's HTTP tool. See the limits below.

---

## 3. What is NOT protected today

Known gaps, each scheduled in the [ROADMAP.md release trains](ROADMAP.md#3-release-trains).
...
| **Egress filtering is coarse on macOS**    | The allowlist is enforced by a local proxy, but macOS sandboxing filters by port, not host: a command connecting to an outside host on the proxy's port is not stopped. Per-host enforcement needs a container on an internal network. Profiles that cannot enforce an allowlist refuse it. | 0.6     |
| **A hang-up goes unseen under Bun**        | The desktop sidecar runs on Bun, which does not signal a client disconnecting. A model call there runs to completion unless cancelled by id (`POST /runs/:id/cancel`, the Stop button) or the kill switch. Under Node a disconnect cancels it.                                              | 0.6     |
| **The Windows key path is untested**       | The DPAPI backend falls back to the key file on any failure, but no Windows runner exercises it.                                                                                                                                                                                            | 0.6     |
| **The sandbox is not wired to a tool yet** | No `exec` tool ships, so nothing calls `packages/sandbox` in production. Container execution itself is untested.                                                                                                                                                                            | 0.6     |
| **DNS rebinding**                          | `http.request` checks resolved addresses, but the connection resolves again, leaving a time-of-check window. Closing it needs a pinned-address dispatcher.                                                                                                                                  | 0.6     |
| **Desktop CSP is disabled**                | `tauri.conf.json` sets `security.csp: null`.                                                                                                                                                                                                                                                | 1.0     |
| **Updater public key is a placeholder**    | Update signatures cannot verify, so updates fail closed. It must be replaced before a release advertises in-app updates.                                                                                                                                                                    | 1.0     |

Release 0.4 closed the four blocking gaps (unauthenticated gateway, all-interface bind,
...
A change that violates one of these does not merge.
