...
> tool executes commands or touches repositories today, so there is nothing for them to guard.
> The 0.6 agent runtime is their first consumer. Everything else below is live in the gateway.
...
- **`packages/policy`** _(library; not yet called by the gateway)_ — deny-by-default capability rules over tool groups, with four profiles (`minimal`, `research`, `coding`, `trusted`), wildcard and `group:`/`risk:` matching, per-run call and runtime budgets, and a `guard()` that fails closed when a call needs approval and no approver is wired.
- **`packages/sandbox`** _(library; not yet called by the gateway)_ — process isolation with three profiles: `native` (development only, warns), `seatbelt` (macOS `sandbox-exec`, writes confined to the run's workspace and a private temp directory), and `container` (Docker/Podman, `--cap-drop ALL --read-only --network none`). Wall-clock kill through the whole process group, output caps, and secret redaction on the way back. Policy says what an agent may ask for; the sandbox decides what a process can physically do — permitting `exec` in an allowlist and then handing the command to `child_process` is not isolation.
- **`packages/workspaces`** _(library; not yet called by the gateway)_ — register local repositories, clone remotes into a content-addressed cache, and give each run its own `git worktree`. Publishing branches, commits with a `Generated-by: OpenClerq` trailer, pushes with `--force-with-lease`, and opens a **draft** pull request on GitHub or GitLab. Git is always invoked argv-only with `GIT_TERMINAL_PROMPT=0`, so no input can become a shell fragment or a credential prompt.
...
- **Approval system** _(inbox and endpoints live; nothing raises an approval until a tool needs one)_. Risky calls raise an approval that is a row in the store, so a decision survives a restart and stays auditable. `GET /approvals`, `POST /approvals/:id/approve|deny`. **Waiting fails closed**: an approval nobody answers is refused, never granted.
- **Kill switch.** `POST /kill` denies every pending approval at once.
- **Egress allowlist, enforced at the boundary** _(part of the sandbox library)_. `network` on a sandbox spec takes `'none'` (default), `'all'`, or a host allowlist (`example.com`, `example.com:443`, `*.example.com`). An allowlist starts a loopback proxy that enforces it, and the seatbelt profile then denies every other outbound port, so the process cannot route around it; refused attempts are recorded on the result. A profile that cannot enforce an allowlist now **refuses** it instead of silently granting full network access, which is what `native` and `container` previously did. macOS can filter only by port, so one residual gap remains and is documented in SECURITY.md.
...
- **Every push reported a failed workflow.** `release-macos-signed.yml` read `secrets` in a step's `if:`, which GitHub does not allow, so it rejected the file and recorded a failed run on every push — though the workflow only runs on tags. The check now reads the secret into a job-level variable, where secrets are allowed.
- **Tagging a release would have failed on macOS.** GitHub passes a secret that is not configured as an empty string, and Tauri then signs with the identity `""` and stops (`no identity found`); with the release's publish step waiting on every build, no release would have been created. The build script treats empty signing variables as unset, so unsigned release builds complete and configured ones still sign.
...
- **A closed tab no longer keeps spending.** The model call behind a request carried on — and was billed — after the client hung up. Every run now carries an abort signal; a hang-up cancels the run and the provider call mid-stream, and the run is recorded as `cancelled`. Send `continueOnDisconnect: true` to have the work finished and recorded regardless. Under Bun — the desktop sidecar — a hang-up cannot be detected at all, so a stream now names its run first and `POST /runs/:id/cancel` stops it on any runtime; the client cancels by id when its caller aborts, and the chat console has a **Stop** button.
- **The kill switch stops things.** `POST /kill` only refused pending approvals, and emitted `gateway.stopped` while nothing stopped. It now also cancels every run in flight and pauses triggers until `POST /resume`; any part can be left out (`{ "triggers": false }`). `GET /kill` reports the state. A cancelled run answers `409 run_cancelled` instead of a `503` that blamed the provider.
- **Chat requests are validated before anything is spent or streamed.** An unknown `mode` was silently treated as raw; a comparison with non-string models answered 200 with an internal error message in the body. Both now answer `400` with a plain reason, including on the streaming path. A model may appear twice in a comparison, to see its variance.
- **A user's message is never reinterpreted.** Assistant metadata was stored inside the message text and guessed back out on read, which rewrote any user message shaped like that JSON. It has its own column now (store migration 3); older assistant rows still read correctly.
- **Model responses are bounded** (8 MiB, `CLERQ_MAX_RESPONSE_BYTES`, or per call). A streamed answer over the limit is cut off and marked `truncated`; a buffered one is refused. Nothing bounded them before.
