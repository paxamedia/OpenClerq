    ...
    >
      <div className="about-modal" onClick={(e) => e.stopPropagation()}>
        <div className="about-header">
          ...
          <button
            type="button"
            className="btn btn-ghost about-close"
            onClick={onClose}
            aria-label="Close"
          >
            ×
        ...
        <div className="about-body">
          <p>
            <strong>OpenClerq</strong> is your desktop helper — like an administrative worker. It
            runs on your computer and keeps your data local.
          </p>
          <h3>Warnings and disclaimer</h3>
          <p>
            <strong>Use at your own risk. No warranty.</strong> This software is provided as-is. The
            authors are not responsible for any loss or damage arising from its use.
          </p>
          <h3>Access and permissions</h3>
          <p>To perform local clerical work, this app is allowed to:</p>
          <ul>
            <li>
              <strong>Read and write</strong> your config and optional API key under{' '}
              <code>~/.clerq</code> (config.json, .env).
            </li>
            <li>
              <strong>Read</strong> paths you choose for modules (e.g. skills directories) and their
              manifest files.
            </li>
            <li>
              <strong>Open links</strong> in your browser (e.g. GitHub, docs) when you click them.
            </li>
            <li>
              If you configure an <strong>API (cloud)</strong> LLM provider: the gateway (which you
              run separately) will send requests to that provider; the desktop app only talks to
              your local gateway.
            </li>
          </ul>
          <p>
            Calculations are performed by a local Rust engine on your machine; no calculation data
            is sent to any server unless you choose a cloud LLM for explanations and tasks.
          </p>
          <h3>Free and open source</h3>
          <p>
            You can see the code, use it for free, and build on it. OpenClerq is{' '}
            <em>inspired by</em>{' '}
            <button type="button" className="link-button" onClick={() => openUrl(OPENCLAW_URL)}>
              OpenClaw
            </button>{' '}
            (same ideas: gateway, skills, local-first) — our implementation is original.{' '}
            <button type="button" className="link-button" onClick={() => openUrl(GITHUB_URL)}>
              OpenClerq on GitHub
            </button>
            .
          </p>
          <h3>This version</h3>
          <p>
            You're using the <strong>open-source OpenClerq desktop app</strong>. Configure your own
            API keys, tools, and modules locally. Any commercial modules you purchase live
            separately and are not bundled with this app.
          </p>
          <div
          ...
          >
            ...
            {updateStatus === 'error' && updateError && (
              <span style={{ fontSize: '0.85rem', color: 'var(--error)' }}>{updateError}</span>
            )}
          </div>
        </div>
      </div>
    ...
    <footer className="footer" style={style}>
      ...
      {onAbout && (
        <>
          ...
          <button type="button" className="btn btn-ghost footer-link" onClick={onAbout}>
            About
          </button>
        </>
      )}
...
function ThemeToggle({ theme, setTheme }: { theme: Theme; setTheme: (t: Theme) => void }) {
  ...
  const label =
    theme === 'auto' ? 'Theme: system' : theme === 'light' ? 'Theme: light' : 'Theme: dark';
  return (
...
function AskResultDisplay({ result }: { result: ExplainResponse | TaskResponse | string }) {
  ...
  const taskTrace = (
    result as unknown as { trace?: { step: string; detail?: string; duration_ms?: number }[] }
  ).trace;
  const hasExtra =
    'intent' in result && (taskResult.skillSlug ?? taskResult.error ?? taskTrace?.length);
  return (
      ...
      <div className="about-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="about-body">
          <dl style={{ margin: 0 }}>
            ...
            <dd>
              <kbd>Ctrl</kbd>+<kbd>Enter</kbd> (or <kbd>⌘</kbd>+<kbd>Enter</kbd> on Mac) run tool
            </dd>
          </dl>
          <p style={{ marginTop: '1rem', fontSize: '0.9rem', color: 'var(--text-muted)' }}>
            Tip: Use Preview context before sending to see what will be sent to the LLM. Use Dry run
            (Task mode) to test without API calls.
          </p>
        </div>
      </div>
    </div>
...
function DeveloperView({
  ...
  const [askContextPreview, setAskContextPreview] = useState<{
    systemPrompt: string;
    userContent: string;
  ...
  const [toolsList, setToolsList] = useState<
    Array<{ name: string; description: string }> | string | null
  >(null);
  ...
  const runAsk = async () => {
    ...
    try {
      ...
      const modelOverride =
        ...
        selectedModel &&
        selectedModel !== availableModels.current
          ? selectedModel
          : undefined;
      if (askMode === 'explain') {
  ...
  const healthResponse =
    gatewayStatus !== null && typeof gatewayStatus !== 'string' ? gatewayStatus : null;
    ...
    <div className="app app--developer">
      ...
      <div className="dev-panels">
        <div className="dev-panel">
          ...
          {showBuilderTools && <CalculatorPanel />}

          {showBuilderTools && (
            <CollapsibleSection
              title="Tools (core)"
              defaultOpen={true}
              badge={toolsList && typeof toolsList !== 'string' ? toolsList.length : undefined}
            >
              <p className="section-desc">
                Built-in generic tools. Test capabilities like filesystem reads before wiring into
                modules or skills.
              </p>
              <div
                className="row"
                style={{ marginBottom: '0.75rem', gap: '0.5rem', flexWrap: 'wrap' }}
              >
                <button
                  type="button"
                  className="btn"
              ...
              {!toolsList ? (
                <p style={{ fontSize: '0.9rem', color: 'var(--text-muted)' }}>
                  Click Load tools to list available tools.
                </p>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                ...
                >
                  ...
                  <button
                    ...
                    onClick={() => {
                      setToolName('fs.read');
                      setToolInput('{"relativePath": "README.md"}');
                    }}
                  >
                    fs.read README
                  </button>
                  <button
                    ...
                    onClick={() => {
                      setToolName('fs.read');
                      setToolInput('{"relativePath": "package.json"}');
                    }}
                  >
                ...
                {toolResult !== null && (
                  <ResultBox
                    error={toolResult.startsWith('Gateway') || toolResult.startsWith('Error')}
                  >
                    {toolResult}
                  </ResultBox>
                )}
          ...
          <section className="section dev-section">
            ...
            >
              ...
              >
                <input
                  type="radio"
                  checked={askMode === 'task'}
                  onChange={() => setAskMode('task')}
                />
                Task
              </label>
              <label
              ...
              >
                <input
                  type="radio"
                  checked={askMode === 'explain'}
                  onChange={() => setAskMode('explain')}
                />
                Explain
              </label>
              {askMode === 'task' && (
                <label
                  ...
                  title="Parse intent only, no LLM call"
                >
                  <input
                    type="checkbox"
                    checked={askDryRun}
                    onChange={(e) => setAskDryRun(e.target.checked)}
                  />
                  Dry run
                </label>
              )}
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      setAskContextPreview({
                        systemPrompt: '',
                        userContent: String(e),
              ...
              <CollapsibleSection title="Context window preview" defaultOpen={true}>
                <p className="section-desc" style={{ marginBottom: '0.5rem' }}>
                  {askContextPreview.estimatedInputTokens != null && (
                    <span style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>
                      ~{askContextPreview.estimatedInputTokens} input tokens ·{' '}
                    </span>
                  )}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                  <div>
                    ...
                    >
                      {askContextPreview.systemPrompt}
                    </pre>
                  </div>
                  <div>
                  ...
                  <button
                    ...
                    onClick={() => setAskContextPreview(null)}
                  >
                    Close
                  </button>
                </div>
            ...
            {askResult !== null && <AskResultDisplay result={askResult} />}
          </section>
          {connectionStatus === 'connected' && (
            <LogsPanel connected={connectionStatus === 'connected'} />
          )}
            ...
            <section className="section dev-section observability-section">
              ...
              <div className="observability-metrics">
                ...
                {typeof metrics.llm_calls_total === 'number' && metrics.llm_calls_total > 0 && (
                  <>
                    <span>
                      <strong>LLM calls</strong> {metrics.llm_calls_total}
                    </span>
                    {metrics.llm_avg_latency_ms != null && (
                      <span>
                        <strong>Avg latency</strong> {metrics.llm_avg_latency_ms}ms
                      </span>
                    )}
                    {(metrics.llm_input_tokens_total ?? 0) +
                    ...
                    {(metrics.llm_failure_rate ?? 0) > 0 && (
                      <span>
                        <strong>Failure rate</strong> {(metrics.llm_failure_rate ?? 0) * 100}%
                      </span>
                    )}
                  </>
                )}
          ...
          {showBuilderTools && (
            ...
            >
              <p className="section-desc" style={{ marginBottom: 0 }}>
                Features: memory layer, triggers, capabilities, secrets vault, observability,
                builder/operator modes.
                <button
                  type="button"
                  className="link-button"
                  style={{ marginLeft: 4 }}
                  onClick={() => openUrl('https://github.com/paxamedia/OpenClerq')}
                >
  ...
  return (
    <div className="app">
      ...
      <section className="section">
        <p className="section-desc">
          This mode is not implemented yet. Use Developer to configure and run tasks.
        </p>
      </section>
      <AppFooter onAbout={onAbout} />
    </div>
...
function App() {
  ...
  const [loadedModuleManifests, setLoadedModuleManifests] = useState<
  ...
  const reloadModules = useCallback(async (paths: ModulePathEntry[] | undefined) => {
    ...
    setLoadedModuleManifests(
      loaded.map((m) => ({ id: m.id, name: m.name, description: m.description }))
    );
  }, []);
  ...
  const modulesForPicker = allModules.filter((m) => m.enabled !== false).length
    ? allModules.filter((m) => m.enabled !== false)
    : DEFAULT_MODULES;
    ...
    const modesWithDeveloper: ModuleEntry[] = [
      {
        id: 'developer',
        name: 'Control Tower',
        description: 'Gateway, tools, skills, Ask — configure and run tasks',
        enabled: true,
      },
  ...
  if (view === 'module' && selectedModule) {
    const displayName =
      (config?.modules?.length ? config.modules : DEFAULT_MODULES).find(
